...
window.initiateCampaign = function(campaignType) {
  ...
  const campaign = {
    ...
    currentStage: 1,
    completedMissions: [],
    stageProgress: {}, // Track progress for each stage
    state: "active", // active, completed, failed
  };
  
  // Initialize stage progress
  template.missionProgression.forEach(stage => {
    campaign.stageProgress[stage.stage] = {
      completed: false,
      missionsCompleted: 0,
      requiredCompletions: stage.requiredCompletions
    };
  });
...
window.updateCampaignProgress = function(completedMissionType) {
  ...
  if (!campaign.completedMissions.includes(completedMissionType)) {
    ...
    if (currentStageTemplate.availableMissions.includes(completedMissionType)) {
      campaign.stageProgress[campaign.currentStage].missionsCompleted++;
      
      // Check if stage is complete
      if (campaign.stageProgress[campaign.currentStage].missionsCompleted >= 
          campaign.stageProgress[campaign.currentStage].requiredCompletions) {
        
        // Mark current stage as completed
        campaign.stageProgress[campaign.currentStage].completed = true;
        ...
        if (!nextStage) {
          ...
          // Update UI to show new missions
  ...
  // Save campaign state
  window.saveCampaignState();
};

// Save campaign state
window.saveCampaignState = function() {
  if (window.gameState.currentCampaign) {
    localStorage.setItem('currentCampaign', JSON.stringify(window.gameState.currentCampaign));
  }
};

// Load campaign state
window.loadCampaignState = function() {
  const savedCampaign = localStorage.getItem('currentCampaign');
  if (savedCampaign) {
    try {
      window.gameState.currentCampaign = JSON.parse(savedCampaign);
    } catch (e) {
