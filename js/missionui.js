...
window.showMissionSelectionScreen = function() {
    ...
    // Track unique missions to prevent duplicates
    const addedMissions = new Set();
    
    // Available missions
    currentStage.availableMissions.forEach(missionType => {
        // Skip if we've already added this mission type
        if (addedMissions.has(missionType)) return;
        addedMissions.add(missionType);
        ...
        if (!alreadyCompleted) {
            // Use data attribute to store mission type
            missionButton.setAttribute('data-mission-type', missionType);
            missionButton.addEventListener('click', function(e) {
                const missionType = e.target.getAttribute('data-mission-type');
                if (missionType && typeof window.startMission === 'function') {
