...
window.initializeMission = function(missionType) {
    ...
    const mission = {
      ...
      missionTimeContext: window.TimeManager ? window.TimeManager.startMissionTime() : { currentTime: 0 }
    };
...
window.startMission = function(missionType) {
    ...
    if (window.gameState.inMission) {
        window.showNotification("Already in a mission", 'warning');
        return false;
    }
    ...
    window.gameState.savedCampState = {
        ...
        time: window.TimeManager ? window.TimeManager.getCurrentTime() : 0,
    ...
    const campaignElements = document.querySelectorAll('.campaign-element');
    campaignElements.forEach(el => el.style.display = 'none');
    
    // Show mission UI elements
    const missionInterface = document.getElementById('missionInterface');
    if (missionInterface) {
        missionInterface.classList.remove('hidden');
        missionInterface.classList.add('active');
    }
    ...
    if (typeof window.showMissionBriefing === 'function') {
        window.showMissionBriefing(mission);
    }
    
    // Update action buttons for mission
    if (typeof window.updateMissionActionButtons === 'function') {
...
window.processMissionDay = function() {
    ...
    for (const encounter of todaysEncounters) {
      // Handle encounter
      if (typeof window.processMissionEncounter === 'function') {
          window.processMissionEncounter(encounter);
      }
      
      // If mission failed, exit early
      if (mission.state === "failed") {
        window.handleMissionFailure(mission);
        return;
      }
    }
    ...
    if (mission.currentDay > mission.duration) {
      ...
      window.handleMissionFailure(mission, "time_expired");
    } else {
      // Update mission UI
      if (typeof window.updateMissionUI === 'function') {
          window.updateMissionUI(mission);
...
window.handleMissionFailure = function(mission, reason = "defeat") {
    ...
    // Add failure event to mission log
    mission.events.push({
        day: mission.currentDay,
        type: "failure",
        description: `Mission failed: ${reason}`
    });
    
    // Show failure message
    window.addToNarrative(`Mission failed: ${reason}. You must return to camp.`);
    
    // Return to camp
    window.returnToCamp(false);
};

// Return to camp
window.returnToCamp = function(success = true) {
    const mission = window.gameState.currentMission;
    if (!mission) return;
    
    // Restore saved camp state
    if (window.gameState.savedCampState) {
        window.gameState.health = window.gameState.savedCampState.health;
        window.gameState.stamina = window.gameState.savedCampState.stamina;
        window.gameState.morale = window.gameState.savedCampState.morale;
        window.gameState.playerInjuries = [...window.gameState.savedCampState.injuries];
        
        // Restore time if time manager exists
        if (window.TimeManager && window.gameState.savedCampState.time) {
            window.TimeManager.setTime(window.gameState.savedCampState.time);
        }
    ...
    const missionInterface = document.getElementById('missionInterface');
    if (missionInterface) {
        missionInterface.classList.remove('active');
        missionInterface.classList.add('hidden');
    }
    
    // Show campaign elements
    const campaignElements = document.querySelectorAll('.campaign-element');
    campaignElements.forEach(el => el.style.display = '');
    
    // Clear mission state
    window.gameState.inMission = false;
    window.gameState.currentMission = null;
    
    // Update narrative based on success/failure
    if (success) {
        window.addToNarrative("You return to camp, mission complete.");
    } else {
        window.addToNarrative("You return to camp, mission failed.");
        window.gameState.morale = Math.max(0, window.gameState.morale - 10);
    }
...
console.log("Mission System loaded successfully");
