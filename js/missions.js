...
window.showMissionSelectionScreen = function() {
    ...
    stageDescription.textContent = `Select a mission to advance the campaign. You need to complete ${currentStageTemplate.requiredCompletions} missions at this stage.`;
    stageDescription.style.marginBottom = '20px';
    actionsContainer.appendChild(stageDescription);
    
    // Track mission types to avoid duplicates
    const displayedMissionTypes = new Set();
    
    // Available missions
    currentStage.availableMissions.forEach(missionType => {
      // Skip if we've already displayed this mission type
      if (displayedMissionTypes.has(missionType)) return;
      displayedMissionTypes.add(missionType);
